...
module RequestSchemas {
	...
	export const Response = Type.Object({
		schemas: Type.Array(
			Type.Tuple([BusEndpoints, Type.Array(RouteSchema)], {
				description:
					"Where first item in array describes bus endpoints, the second describes all routes included",
			}),
			{
				minItems: 1,
				maxItems: 2,
...
type ResponseSchemas = Static<typeof RequestSchemas.Response>["schemas"]
...
export async function busLineRouteSchema(req: FastifyRequest<Request>, res: FastifyReply) {
	...
	const schemasCollection = result.reduce<ResponseSchemas>((acc, curr) => {
		const endpoint = acc.find(
			(endpoint) => endpoint[0].start_bus_stop.id === curr.Route.StartBusStop.Id
		)
		if (!endpoint) {
			acc.push([
				{
			...
			endpoint[1].push(adapt(curr))
		}
		return acc
	}, [])

	res.code(200).send({ schemas: schemasCollection })
}
